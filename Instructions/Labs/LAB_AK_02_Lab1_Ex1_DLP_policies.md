...
7. **정책을 적용할 위치 선택** 페이지에서 **Teams 채팅 및 채널 메시지** 옵션만 사용하도록 선택하고 **다음**을 선택합니다.
...
16. **규칙 만들기** 페이지에서 **+ 작업 추가**를 선택하고 **Microsoft 365 위치에서 액세스 제한 또는 콘텐츠 암호화**를 선택합니다.

17. **Microsoft 365 위치에서 액세스 제한 또는 콘텐츠 암호화** 앞에 있는 체크박스를 선택하고 **모든 사람. 모든 사람, 콘텐츠 소유자, 마지막으로 수정한 사람 및 사이트 관리자만 계속 액세스할 수 있습니다.**를 선택합니다.

18. **규칙 만들기** 페이지의 **사용자 알림** 섹션에서 스위치를 선택하여 **켜기** 위치로 이동합니다.

19. **규칙 만들기** 페이지의 **사용자 재정의** 섹션에서 스위치를 선택하여 **켜기** 위치로 이동하고 **재정의하려면 비즈니스 사유 필요** 옵션을 선택합니다.

20. **인시던트 보고서** 섹션의 **관리 경고 및 보고서에서 이 심각도 수준 사용** 드롭다운에서 **낮음**을 선택합니다.

21. **인시던트 보고서** 섹션에서 **규칙이 일치하는 경우 관리자에게 알림을 보냅니다.** 스위치를 선택하여 **켜기** 위치로 이동하고 옵션을 검토합니다. 기본 설정을 적용하는 경우 정책을 만드는 사용자에게 알림이 표시됩니다.

22. **저장**, **다음**을 차례로 선택합니다.

23. **정책 테스트 또는 켜기** 페이지에서 **먼저 테스트하겠습니다.**를 선택하고 **테스트 모드에 있을 때 정책 팁 표시**를 선택합니다.

24. **다음**을 선택하고 정책 구성을 검토합니다.

25. **제출**을 선택하여 정책을 만듭니다.
...
6. **정책을 적용할 위치 선택** 페이지에서 **Exchange 전자 메일** 옵션을 사용하도록 선택하고 **정책을 검토하고 만듭니다.** 가 표시될 때까지 계속 **다음**을 선택합니다.
...
5. PowerShell에 다음 명령을 입력하여 4단계에서 만든 DLP 정책에 DLP 규칙을 추가합니다.
...
5. **정책 테스트 또는 켜기** 페이지가 표시될 때까지 계속 **다음**을 선택한 후 해당 페이지가 표시되면 **예, 지금 켜겠습니다.**를 선택합니다.
...
### 작업 6 - MCAS(Microsoft Cloud App Security)에서 파일 모니터링을 사용하도록 설정

Microsoft Cloud App Security에서 파일 정책을 사용하여 비즈니스용 OneDrive 및 SharePoint Online 위치에서 파일을 보호하려고 합니다. 파일 정책을 만들려면 MCAS에서 조직의 파일을 검사할 수 있도록 파일 모니터링을 사용하도록 설정해야 합니다.

1. 클라이언트 1 VM(LON-CL1)에 **lon-cl1\admin** 계정으로 로그인되어 있는 상태여야 합니다.

2. **Microsoft Edge**에서 **https://portal.cloudappsecurity.com** 으로 이동한 다음 **MOD 관리자**로 Cloud App Security 포털에 로그인합니다. 로그인 ID로는 admin@WWLxZZZZZZ.onmicrosoft.com을 사용합니다(여기서 ZZZZZZ는 랩 호스팅 공급자가 제공한 고유 테넌트 ID임).  관리자의 암호는 랩 호스팅 공급자가 제공합니다.

3. 오른쪽 위의 프로필 정보 옆에 있는 톱니 바퀴 모양 **설정** 아이콘을 선택하고 드롭다운 메뉴에서 **설정**을 선택합니다.

4. **설정** 페이지의 **정보 보호** 아래에서 **파일**을 선택합니다.

5. **파일 모니터링 사용** 체크박스를 선택하고 **저장**을 선택합니다.

MCAS에서 파일 모니터링을 사용하도록 설정했습니다. 이제 파일 정책을 사용하여 파일에서 중요한 콘텐츠를 검사할 수 있습니다.

### 작업 7 - MCAS용 파일 정책 만들기

이 작업에서는 Microsoft Cloud App Security에서 파일 정책을 만듭니다. 이 정책은 비즈니스용 OneDrive 및 SharePoint Online에서 파일을 검사한 다음 신용 카드 정보가 포함된 파일이 공유되면 자동으로 해당 파일을 격리합니다.
...
2. 그리고 **Microsoft Edge**에는 Cloud App Security 포털 탭이 계속 열려 있어야 합니다. 해당 탭이 열려 있으면 탭을 선택하고 다음 단계를 진행합니다. 해당 탭을 닫았다면 새 탭에서 **https://portal.cloudappsecurity.com** 으로 이동합니다.

3. **Microsoft Cloud App Security 포털**의 왼쪽 탐색 창에서 **컨트롤**을 확장하고 **정책**을 선택합니다.

4. **정책** 페이지에서 **정책 만들기**를 확장하고 **파일 정책**을 선택합니다.

5. **파일 정책 만들기** 페이지의 **정책 이름** 필드에 *파일의 신용 카드 정보*를 입력하고 **설명** 필드에는*파일의 신용 카드 번호를 공유할 수 없도록 보호하는 정책입니다.*를 입력합니다.

6. **정책 심각도**는 **낮음**으로 유지하고 **범주**가 **DLP**로 설정되어 있는지 확인합니다. 파일 정책의 경우 DLP가 기본값입니다.

7. **이 정책을 적용할 파일용 필터 만들기** 영역에서 **공용(인터넷), 외부, 공용** 드롭다운 메뉴를 확장하고 **내부**를 추가합니다.
...
이제 비즈니스용 OneDrive와 SharePoint에 저장된 파일을 지속적으로 검사해 신용 카드 정보를 찾은 다음 조직 내부에서 해당 정보가 공유되는 경우 격리하는 파일 정책을 만들었습니다.
...
3. **Power Platform 관리 센터**의 왼쪽 탐색 창에서 **데이터 정책**을 선택합니다.
...
8. **범위 정의** 페이지에서 **모든 환경 추가**를 선택하고 **다음**을 선택합니다.

9. **정책 검토 및 만들기** 페이지에서 정책 설정을 검토하고 **정책 만들기**를 선택합니다.
